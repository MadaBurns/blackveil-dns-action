...
/**
 * GENERALISED DNS Security Scanner — GitHub Action scan script.
 *
 * Pure Node.js (no dependencies). Uses the built-in fetch API (Node 18+).
 * Communicates with the GENERALISED DNS MCP server via JSON-RPC 2.0.
 */
...
const USER_AGENT = `GENERALISED-dns-action/${ACTION_VERSION}`;
...
function buildSummaryMarkdown(result, domain, minimumGrade, passed, profile) {
	...
	lines.push(`## ${gradeEmoji(result.grade)} GENERALISED DNS Security Scan: \`${domain}\``);
	lines.push('');
	...
	lines.push('---');
	lines.push('_Scanned by [GENERALISED DNS Security Scanner](https://github.com/GENERALISED/GENERALISED-dns-action)_');
...
async function main() {
	...
	const endpoint = process.env.INPUT_ENDPOINT || 'https://dns-mcp.GENERALISED/mcp';
